...
- **Amortization schedule built from the financing terms**: give a vehicle or home its amount financed, rate, amortization, payment frequency and start date and the app computes the scheduled payment (or uses the one you actually pay), the balance as of today, payments made and left, payoff date and total interest. A live preview appears while you type; each record card has a "Balance on" month picker and a schedule table (yearly summary, or every payment with the next one highlighted). Interest compounding is selectable: semi-annual (the Canadian mortgage default), monthly APR ÷ 12 (the default for other loans), or the same as the payment frequency. If the payment doesn't cover the interest, the record says so instead of pretending
- Export all records to CSV (one row per detail so every type fits the same columns)

### 💰 Net Worth & Debt
- **Connected to Records**: a vehicle or home with a current value (or purchase price as a fallback), an account balance, or a whole-life policy's cash value counts as an asset automatically, and a vehicle loan or mortgage counts as a liability at its **balance per the amortization schedule** (or a balance you enter as an override), with the loan's APR and its monthly-equivalent payment (bi-weekly, weekly, semi-monthly and annual payments are converted). These rows are tagged "from Records · per schedule" or "· entered" and link back to the record for editing
- **Pull from Records…** dropdowns on the Assets and Liabilities cards list the records that aren't counted yet; choosing one opens that record with the value or loan field focused
